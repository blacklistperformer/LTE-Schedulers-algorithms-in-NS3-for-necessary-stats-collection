...
uint16_t ue_rnti = 2;
std::map <uint64_t, double> instantaneous_throughput;
    ...
    const FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
    ...
    for (auto itStats = m_flowStatsDl.begin(); itStats != m_flowStatsDl.end(); itStats++)
    {
        ...
        // update Instantaneous througput
        uint64_t sim_time = Simulator::Now().GetMilliSeconds();
        if (itStats->first == ue_rnti) {
            double i_throughput = (*itStats).second.lastAveragedThroughput;
            instantaneous_throughput.insert(std::pair<uint64_t, double>(sim_time, i_throughput));
        }
